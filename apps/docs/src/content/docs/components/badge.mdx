...
import OffsetVue from '~/components/demos/badge/OffsetVue.vue';
import OffsetVueSource from '~/components/demos/badge/OffsetVue.vue?raw';
...
`offset` 是在 `placement` 基础上叠加 `[x, y]` 像素偏移 —— 用来贴合不规则子元素（如圆形头像、带阴影的按钮）。正值方向：x 向右、y 向下。

<DemoFrame
  vueCode={`<CfBadge :content="3" :offset="[-4, 4]">
  <CfButton variant="tertiary">向内</CfButton>
</CfBadge>
<CfBadge :content="3" :offset="[6, -6]">
  <CfButton variant="tertiary">向外</CfButton>
</CfBadge>`}
  reactCode={`<CfBadge content={3} offset={[-4, 4]}>
  <CfButton variant="tertiary">向内</CfButton>
</CfBadge>
<CfBadge content={3} offset={[6, -6]}>
  <CfButton variant="tertiary">向外</CfButton>
  ...
  vueSource={OffsetVueSource}>
  <OffsetVue client:load />
</DemoFrame>
