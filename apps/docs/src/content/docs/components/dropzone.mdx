...
|---|---|---|---|
| `update:modelValue` | `onChange` | `File[]` | 文件列表变化时触发（新增或移除） |
| `change` | `onChange` | `File[]` | 与 `update:modelValue` 同步触发，载荷相同 |
| `reject` | `onReject` | `DropzoneRejection[]` | 一批被拒绝的文件，每项含 `{ file, reason }` |
| `remove` | `onRemove` | `(file: File, index: number)` | 通过内置列表移除单个文件时触发 |
...
interface DropzoneRejection {
  file: File;
  reason: 'too-large' | 'too-many' | 'wrong-type' | 'duplicate';
}
```
...
|---|---|
| `default` | 替换标题区域 |
| `icon` | 替换默认上传插图（默认来自 `CfStatusIllustration variant="upload"`） |
