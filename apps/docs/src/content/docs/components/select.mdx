---
title: Select 选择器
description: 下拉选择 —— 单/多选、可搜索、分组、loading 异步占位、键盘导航、tag 标签溢出收缩。
---

import DemoFrame from '~/components/DemoFrame.astro';
import BasicVue from '~/components/demos/select/BasicVue.vue';
import BasicVueSource from '~/components/demos/select/BasicVue.vue?raw';
import VariantsVue from '~/components/demos/select/VariantsVue.vue';
import VariantsVueSource from '~/components/demos/select/VariantsVue.vue?raw';
import AdvancedVue from '~/components/demos/select/AdvancedVue.vue';
import AdvancedVueSource from '~/components/demos/select/AdvancedVue.vue?raw';
import EventsVue from '~/components/demos/select/EventsVue.vue';
import EventsVueSource from '~/components/demos/select/EventsVue.vue?raw';
import MultiVue from '~/components/demos/select/MultiVue.vue';
import MultiVueSource from '~/components/demos/select/MultiVue.vue?raw';
import SearchVue from '~/components/demos/select/SearchVue.vue';
import SearchVueSource from '~/components/demos/select/SearchVue.vue?raw';
import GroupVue from '~/components/demos/select/GroupVue.vue';
import GroupVueSource from '~/components/demos/select/GroupVue.vue?raw';
import AsyncLoadVue from '~/components/demos/select/AsyncLoadVue.vue';
import AsyncLoadVueSource from '~/components/demos/select/AsyncLoadVue.vue?raw';

## 基础用法

通过 `options` 数组传入选项，每项至少包含 `value` 与 `label`。

<DemoFrame
  vueCode={`<script setup lang="ts">
import { ref } from 'vue';
import { CfSelect, type SelectOption } from '@chufix-design/vue';

const options: SelectOption[] = [
  { value: 'beijing', label: '北京' },
  { value: 'shanghai', label: '上海' },
  { value: 'guangzhou', label: '广州' },
];

const city = ref<string | null>('shanghai');
</script>

<template>
  <CfSelect v-model="city" :options="options" placeholder="选一个城市" />
</template>`}
  reactCode={`import { useState } from 'react';
import { CfSelect, type SelectOption } from '@chufix-design/react';

const options: SelectOption[] = [
  { value: 'beijing', label: '北京' },
  { value: 'shanghai', label: '上海' },
  { value: 'guangzhou', label: '广州' },
];

export default function Demo() {
  const [city, setCity] = useState<string | null>('shanghai');
  return (
    <CfSelect
      value={city}
      options={options}
      placeholder="选一个城市"
      onChange={(v) => setCity(v as string | null)}
    />
  );
}`}

  vueSource={BasicVueSource}>
  <BasicVue client:load />
</DemoFrame>

## 尺寸

3 档尺寸控制高度与字号，与同行的 Input / Textarea 保持一致。

import SizesVue from '~/components/demos/select/SizesVue.vue';
import SizesVueSource from '~/components/demos/select/SizesVue.vue?raw';

<DemoFrame
  vueCode={`<CfSelect v-model="v1" :options="options" size="sm" placeholder="size = sm" />
<CfSelect v-model="v2" :options="options" size="md" placeholder="size = md" />
<CfSelect v-model="v3" :options="options" size="lg" placeholder="size = lg" />`}
  reactCode={`<CfSelect value={v1} options={options} size="sm" placeholder="size = sm" onChange={setV1} />
<CfSelect value={v2} options={options} size="md" placeholder="size = md" onChange={setV2} />
<CfSelect value={v3} options={options} size="lg" placeholder="size = lg" onChange={setV3} />`}

  vueSource={SizesVueSource}>
  <SizesVue client:load />
</DemoFrame>

## 视觉变体

3 种 variant 与 Input 一致：`outline`（默认）/ `filled` / `ghost`。

<DemoFrame
  vueCode={`<CfSelect v-model="v1" :options="options" variant="outline" />
<CfSelect v-model="v2" :options="options" variant="filled" />
<CfSelect v-model="v3" :options="options" variant="ghost" />`}
  reactCode={`<CfSelect value={v1} options={options} variant="outline" onChange={setV1} />
<CfSelect value={v2} options={options} variant="filled" onChange={setV2} />
<CfSelect value={v3} options={options} variant="ghost" onChange={setV3} />`}

  vueSource={VariantsVueSource}>
  <VariantsVue client:load />
</DemoFrame>

## 状态与可清空

`clearable` 在已选中时显示 ×；`disabled` 整体禁用；`error` 切到错误色边框。单个 option 可以加 `disabled: true` 跳过。

<DemoFrame
  vueCode={`<CfSelect v-model="a" :options="options" placeholder="可清空" clearable />
<CfSelect v-model="b" :options="options" placeholder="禁用" disabled />
<CfSelect v-model="b" :options="options" placeholder="错误状态" error />`}
  reactCode={`<CfSelect value={a} options={options} clearable onChange={setA} />
<CfSelect value={b} options={options} disabled />
<CfSelect value={b} options={options} error />`}

  vueSource={AdvancedVueSource}>
  <AdvancedVue client:load />
</DemoFrame>

## 事件与表单

`Select` 除了值变化，也会暴露打开状态、active 选项、清空、焦点等事件；传入 `name` 时会同步一个 hidden input，方便在原生表单里提交当前值。

<DemoFrame
  vueCode={`<CfSelect
  v-model="value"
  :options="options"
  clearable
  name="status"
  @change="(value, meta) => console.log(value, meta.option)"
  @select="(option) => console.log('select', option)"
  @clear="() => console.log('clear')"
  @open-change="(open) => console.log('open', open)"
  @active-change="(option, index) => console.log(option, index)"
/>`}
  reactCode={`<CfSelect
  value={value}
  options={options}
  clearable
  name="status"
  onChange={(value, meta) => console.log(value, meta.option)}
  onSelect={(option) => console.log('select', option)}
  onClear={() => console.log('clear')}
  onOpenChange={(open) => console.log('open', open)}
  onActiveChange={(option, index) => console.log(option, index)}
/>`}

  vueSource={EventsVueSource}>
  <EventsVue client:load />
</DemoFrame>

## 多选

`multiple` 让 model 变成数组，触发器渲染为可移除的 tag 组。`maxTagCount` 把多余的 tag 收成 `+N` 胶囊。退格键删除最后一个 tag。

<DemoFrame
  vueSource={MultiVueSource}>
  <MultiVue client:only="vue" />
</DemoFrame>

## 可搜索

`searchable` 在菜单顶部加一个输入框，按 label 实时过滤。和 multiple 可叠加 —— 像传统 multi-select 一样组合使用。

<DemoFrame
  vueSource={SearchVueSource}>
  <SearchVue client:only="vue" />
</DemoFrame>

## 分组

给 option 加 `group: string`，组件自动按 group 渲染分组标题（顺序按 group 首次出现的次序）。

<DemoFrame
  vueSource={GroupVueSource}>
  <GroupVue client:only="vue" />
</DemoFrame>

## 异步加载

`loading=true` 时触发器变为禁用并展示 spinner，菜单内显示 "加载中…"。结合 onMounted / SWR / TanStack Query 等数据源使用。

<DemoFrame
  vueSource={AsyncLoadVueSource}>
  <AsyncLoadVue client:only="vue" />
</DemoFrame>

## 键盘交互

| 按键              | 行为                                |
| ----------------- | ----------------------------------- |
| `↓` / `↑`         | 打开菜单 / 在选项之间移动 active    |
| `Enter` / `空格`  | 打开菜单 / 选中当前 active 项       |
| `Esc`             | 关闭菜单                            |
| `Tab`             | 关闭菜单并把焦点交还浏览器          |

## API

| Prop          | 类型                               | 默认值       | 说明                                    |
| ------------- | ---------------------------------- | ------------ | --------------------------------------- |
| `options`     | `SelectOption[]`                   | `[]`         | 选项数组，`{ value, label, disabled? }` |
| `placeholder` | `string`                           | `'请选择'`   | 未选中时显示                            |
| `variant`     | `'outline' \| 'filled' \| 'ghost'` | `'outline'`  | 视觉变体                                |
| `size`        | `'sm' \| 'md' \| 'lg'`             | `'md'`       | 整体尺寸                                |
| `clearable`   | `boolean`                          | `false`      | 已选中时显示清除按钮                    |
| `disabled`    | `boolean`                          | `false`      | 整体禁用                                |
| `error`       | `boolean`                          | `false`      | 错误态                                  |
| `name`        | `string`                           | —            | 生成 hidden input，参与原生表单提交     |
| `id`          | `string`                           | —            | 触发按钮 id，并用于关联 listbox         |
| `multiple`    | `boolean`                          | `false`      | 多选模式，model 变为 `Array<value>`     |
| `searchable`  | `boolean`                          | `false`      | 菜单顶部加搜索输入框                    |
| `loading`     | `boolean`                          | `false`      | 异步加载中（trigger 禁用 + spinner）    |
| `emptyText`   | `string`                           | `'无选项'`   | 自定义"无选项"文案                      |
| `maxTagCount` | `number`                           | —            | 多选模式下最多展示几个 tag，溢出收成 +N |

## Events

| Vue 事件 | React 回调 | payload | 说明 |
|---|---|---|---|
| `change` | `onChange` | `(value, { option })` | 值变化，清空时 `option` 为 `null` |
| `select` | `onSelect` | `option` | 选中某个 option |
| `clear` | `onClear` | — | 点击清除按钮 |
| `open-change` | `onOpenChange` | `open` | 下拉菜单打开 / 关闭 |
| `active-change` | `onActiveChange` | `(option, index)` | 键盘或鼠标移动 active option |
| `search` | `onSearch` | `term` | 搜索框输入（仅 `searchable=true` 时） |
| `focus` / `blur` | `onFocus` / `onBlur` | `FocusEvent` | 触发按钮焦点事件 |

> 当前实现使用 `position: absolute` 浮层；如果父级 `overflow: hidden` 裁掉了菜单，后续会扩展 Portal 版本。
