...
export interface Terrain3DCamera {
  /** Pitch from horizontal in degrees. 0 = top-down (looking straight down). Default 60. */
  pitch?: number;
  /** Yaw rotation around Z in degrees. Default 0. */
  yaw?: number;
...
export interface Terrain3DProps {
  /** Flat array of height values, row-major (cols × rows). */
  heightData: number[] | Float32Array;
  /** Grid columns. */
  width: number;
  /** Grid rows. */
  height: number;
  /** Geographic bounds for axis labels (optional, default 0..width × 0..height). */
  bounds?: Terrain3DBounds;
  /** Override min height for color scale (auto from data when omitted). */
  minHeight?: number;
  /** Override max height for color scale. */
  maxHeight?: number;
  /** Vertical exaggeration. Default 1. Larger values exaggerate Z. */
  exaggeration?: number;
  /** Built-in color scale or custom callback (t in [0,1] → rgb in [0,1]). */
  colorScale?: Terrain3DColorScale | ((t: number) => { r: number; g: number; b: number });
  /** Show base wireframe overlay. */
  wireframe?: boolean;
  /** Camera config. */
  camera?: Terrain3DCamera;
  /** Auto-rotate around the scene. */
  autoRotate?: boolean;
  /** Show coordinate grid helper. */
  showGrid?: boolean;
  /** Scene background color (CSS color string). */
  background?: string;
  /** Container width / height. Defaults 100% × 480px. */
  rendererWidth?: number | string;
  rendererHeight?: number | string;
}

/**
 * Built-in color ramps used when consumer passes `colorScale: 'depth' | 'elevation' | 'delta' | 'viridis'`.
 * Each takes t in [0,1] and returns { r, g, b } in [0,1].
 */
...
export function resolveColorScale(
  scale: Terrain3DProps['colorScale'],
