/**
 * Module-singleton drag/drop coordinator (React mirror).
 * Identical semantics to packages/vue/src/composables/dndStore.ts.
 *
 * IMPORTANT: per CLAUDE.md §9.1, consumers must import via the barrel
 * (`from '@chufix-design/react'`) — deep `src/` imports create a second
 * store instance with its own listeners.
 */
